...
def test_la_commune_seule_restreint_aux_logements(monkeypatch):
    monkeypatch.setattr(sources, "_contour_commune", lambda insee: [(47.65, -2.75), (47.66, -2.74), (47.65, -2.75)])
    url = sources.url_seloger("vannes", "56", 47.658, -2.760, 0, insee="56260")
...
def test_la_commune_seule_porte_son_contour(monkeypatch):
    ...
    monkeypatch.setattr(sources, "_contour_commune", lambda insee: contour)
    zone = _zone(sources.url_seloger("vannes", "56", 47.658, -2.760, 0, insee="56260"))
    assert zone["polyline"] == sources._polyline(contour)
...
def test_sans_contour_on_retombe_sur_la_forme_par_commune(monkeypatch):
    ...
    monkeypatch.setattr(sources, "_contour_commune", lambda insee: None)
    url = sources.url_seloger("vannes", "56", 47.658, -2.760, 0, insee="56260")
    assert url == "https://www.seloger.com/immobilier/locations/immo-vannes-56/"


def test_le_rayon_reste_un_cercle():
    zone = _zone(sources.url_seloger("vannes", "56", 47.658, -2.760, 10, insee="56260"))
    assert zone["radius"] == 10
    assert zone["polyline"] == sources._polyline(sources._cercle(47.658, -2.760, 10))
...
def test_le_contour_est_echantillonne_et_referme(monkeypatch):
    ...
    monkeypatch.setattr(sources, "_geojson_commune",
                        lambda insee: {"type": "Polygon", "coordinates": [anneau]})
    points = sources._contour_commune("56260")
    assert len(points) <= sources.CONTOUR_MAX_POINTS + 1
...
def test_une_api_muette_ne_fait_pas_planter(monkeypatch):
    monkeypatch.setattr(sources, "_geojson_commune", lambda insee: None)
    assert sources._contour_commune("56260") is None
