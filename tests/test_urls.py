...
def test_ville_ordinaire_donne_une_seule_recherche():
    urls = scrap.urls_paruvendu("Vannes", "vannes", "56000", "56260", km=10)
    assert len(urls) == 1
...
def test_paris_donne_une_recherche_par_arrondissement():
    ...
    urls = scrap.urls_paruvendu("Paris", "paris", "75056", "75056", km=0)
    assert len(urls) == 20
...
def test_lyon_et_marseille_ont_leurs_propres_bases_insee():
    lyon = scrap.urls_paruvendu("Lyon", "lyon", "69123", "69123", km=0)
    assert len(lyon) == 9
    assert "lyon-69001" in lyon[0] and "codeINSEE=69381" in lyon[0]

    marseille = scrap.urls_paruvendu("Marseille", "marseille", "13055", "13055", km=0)
...
def test_le_rayon_passe_par_lol_pas_par_ray():
    ...
    url = scrap.urls_paruvendu("Vannes", "vannes", "56000", "56260", km=10)[0]
