...
def test_polyline_encode_au_format_google():
    ...
    assert sources._polyline([(38.5, -120.2), (40.7, -120.95), (43.252, -126.453)]) \
...
def test_cercle_couvre_le_rayon_demande():
    ...
    points = sources._cercle(*centre, 10)
...
def test_url_porte_le_rayon_demande():
    url = sources.url_seloger("vannes", "56", 47.6559, -2.7603, 10)
    zone = _locations(url)
...
def test_rayon_nul_cherche_la_commune_seule():
    ...
    url = sources.url_seloger("vannes", "56", 47.6559, -2.7603, 0)
...
def test_champs_d_une_annonce():
    ...
    df = sources.annonces_seloger(_fixture("seloger_vannes.html"))
    premiere = df.iloc[0]
...
def test_secteur_est_l_arrondissement_a_paris():
    ...
    df = sources.annonces_seloger(_fixture("seloger_paris.html"))
    assert set(df["Secteur"]) == {"Paris 16e", "Paris 15e"}
    assert df["Secteur"].iloc[0] == secteur.depuis_cp("75016", "Paris")
...
def test_contrat_de_sortie():
    ...
    df = sources.annonces_seloger(_fixture("seloger_vannes.html"))
    assert list(df.columns) == sources.COLONNES
    assert set(df["Source"]) == {"seloger"}
    assert df["DPE"].iloc[0] == sources.DPE_MAP["C"]
...
def test_une_panne_en_cours_de_pagination_ne_perd_pas_les_pages_lues():
    ...
    df = sources.moissonner_seloger(_ContexteFictif(page), "https://exemple",
                                    sources.annonces_seloger)
    assert len(df) == 3
...
def test_page_sans_annonce_rend_le_contrat_vide():
    ...
    df = sources.annonces_seloger("<html><body>rien ici</body></html>")
    assert df.empty
    assert list(df.columns) == sources.COLONNES
