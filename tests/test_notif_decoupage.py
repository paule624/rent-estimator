...
def test_message_court_part_en_un_seul_morceau():
    texte = _message(3)
    assert notif._decouper(texte, limite=1900) == [texte]


def test_message_long_est_reparti_sous_la_limite():
    morceaux = notif._decouper(_message(60), limite=1900)
...
def test_ce_qui_ne_tient_pas_est_compte_pas_effacé():
    ...
    morceaux = notif._decouper(_message(400), limite=1900, max_messages=3)
...
def test_un_bloc_plus_long_que_la_limite_est_tronque_mais_envoyé():
    ...
    morceaux = notif._decouper("x" * 5000, limite=100)
    assert morceaux
    assert all(len(m) <= 100 for m in morceaux)


def test_message_vide_ne_produit_rien():
    assert notif._decouper("", limite=1900) == []


def test_discord_envoie_autant_de_requetes_que_de_morceaux(monkeypatch):
    envoyes = []
    monkeypatch.setattr(notif.config, "get", lambda k: "https://discord.test/webhook")
    monkeypatch.setattr(notif.urllib.request, "urlopen",
                        lambda req, timeout=None: envoyes.append(req.data) or _Reponse())
    notif._notif_discord(_message(60))
    assert len(envoyes) > 1


def test_telegram_envoie_autant_de_requetes_que_de_morceaux(monkeypatch):
    envoyes = []
    monkeypatch.setattr(notif.config, "get", lambda k: "valeur")
    monkeypatch.setattr(notif.urllib.request, "urlopen",
                        lambda req, timeout=None: envoyes.append(req.data) or _Reponse())
    notif._notif_telegram(_message(200))
    assert len(envoyes) > 1
